...
class HomeContainer extends React.Component<IProps, IState> {
      ...
      <ProfileQuery query={USER_PROFILE}>
        {({ data, loading }) => {
          if (data && data.GetMyProfile) {
            ...
            if (user) {
              return (
                <NearbyQueries query={GET_NEARBY_DRIVERS} skip={user.isDriving} onCompleted={this.handleNearbyDrivers}>
                  {() => (
                    <HomePresenter
                      loading={loading}
  ...
  public onAddressSubmit = async () => {
    ...
    if (result !== false) {
      ...
      this.setState(
        ...
        this.createPath
      ); // call createPath only after setState
    }
  };
  ...
  public setPrice = () => {
    ...
    if (distance) {
      this.setState({
        price: Number(parseFloat(distance.replace(",", "")) * 2).toFixed(2)
      });
    }
  };
